...
def InteriorPointBarrier_EqualityInequality(Q, c, A, b, C, d, tol, kmax=1000, rho=.9, mu0=1e4, mumin=1e-9):
    ...
    x,lamb,s,k = _IPBarrier_Worker_EqualityOnly(Q_p1, c_p1, A, b, C, d, x, lamb, s, tol, kmax, rho, mu0, mumin)
    totalk += k
    ...
    x,lamb,s,k = _IPBarrier_Worker_EqualityOnly(Q_p2, c_p2, A, b, C, d, x, lamb, s, tol, kmax, rho, mu0, mumin)
    totalk += k
...
def _IPBarrier_Worker_EqualityInequality(Q, c, A, b, C, d, x, lamb, s, tol, kmax, rho, mu0, mumin):
    """
    ...
    def Jacobian():
        ...
        return np.block([[C,  np.zeros((p,n+2*m))],
                         [-Q, C.T, np.eye(n), A.T, np.zeros((n,m))],
                         [A, np.zeros((m,p+n+m)),-np.eye(m)],
